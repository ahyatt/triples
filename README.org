...
The =triples= module is a standard database module designed for use in other emacs modules.  It works with the =sqlite= module, and provides a simple way of storing entities and their associated schema.  The triples module is well suited to graph-like applications, where links between entities are important.  The module has wrappers for most common operations, but it is anticipated that occasionally client modules would need to make their own =sqlite= calls.  Many database can be handled by the =triples= module.  It is expected that clients supply the database connection.
